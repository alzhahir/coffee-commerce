<?php
    ...
    foreach($prodData as $currProd){
        ...
        <div class="col w-100 align-items-center justify-content-center align-middle text-center" style="display:inline-block;width:224px!important;">
            ...
            <div id="prodNameLabel" title="<?php echo($currProd["name"])?>" class="user-select-none flex-row fw-bold fs-4 pt-2" style="text-overflow:ellipsis;overflow:hidden;white-space:nowrap;"><?php echo($currProd["name"])?></div>
            <div id="prodPriceLabel" class="user-select-none flex-row fw-normal fs-4 pb-2"><?php echo("RM" . $currProd["price"])?></div>
        <?php
            if($currProd["stock"] > 0){
        ?>
            <button id="prodShoppingBtn" data-value="<?php echo($currProd["id"]); ?>" class="btn btn-primary ahvbutton flex-row fw-normal rounded-pill fs-4 align-middle text-center border-0 px-4 py-2">
