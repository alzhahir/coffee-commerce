<?php
    ...
    $iteration = 0;
    foreach($prodData["id"] as $currprod){
        ?>
        <div class="col w-100 align-items-center justify-content-center align-middle text-center" style="display:inline-block;width:224px!important;">
            ...
            <div id="prodNameLabel" title="<?php echo($prodData["name"][$iteration])?>" class="user-select-none flex-row fw-bold fs-4 pt-2" style="text-overflow:ellipsis;overflow:hidden;white-space:nowrap;"><?php echo($prodData["name"][$iteration])?></div>
            <div id="prodPriceLabel" class="user-select-none flex-row fw-normal fs-4 pb-2"><?php echo("RM" . $prodData["price"][$iteration])?></div>
        <?php
            if($prodData["stock"][$iteration] > 0){
        ?>
        ...
        ?>
            <button class="btn btn-primary ahvbutton flex-row fw-normal rounded-pill fs-4 align-middle text-center border-0 px-4 py-2" disabled>
                <span class="material-symbols-outlined align-middle text-center px-0">
                    add_shopping_cart
                </span>
                Out of Stock
